...
feature {COMMANDER}
   ...
   run (command: COLLECTION[STRING])
      ...
      do
         inspect
            command.count
         when 2 then
            property := command.last
            inspect
               property
            when "username", "url" then
               create query.make(command.first, once "unset", property, once "")
               client.call_server(query, agent when_reply(?))
            when "tag" then
               error_and_help(once "Missing argument: tag to unset", command)
            else
         ...
         when 3 then
            ...
            inspect
               property
            when "username", "url" then
               error_and_help(once "Too many arguments", command)
            when "tag" then
               create query.make(command.first, once "unset", property, command.last)
               client.call_server(query, agent when_reply(?))
            else
               error_and_help(once "Unrecognized argument '#(1)'" # command.last, command)
            end
   ...
   help (command: COLLECTION[STRING]): STRING
      do
         Result := once "[
                    [33munset <key> <property> <value>[0m  Unset a key property.
