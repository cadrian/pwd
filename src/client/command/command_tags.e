...
feature {COMMANDER}
   ...
   run (command: COLLECTION[STRING])
      do
         if not command.is_empty then
            ...
            client.call_server(create {QUERY_TAGS}.make, agent when_reply(?))
         end
      end
   ...
   help (command: COLLECTION[STRING]): STRING
      do
         Result := once "[33mtags[0m               List the known key tags."
      end

feature {}
   when_reply (a_reply: MESSAGE)
      local
         reply: REPLY_TAGS; string: STRING
      do
